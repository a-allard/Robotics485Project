...
extern "C"
{
  ...
  unsigned char * pixy_getImage(struct rBlock * blk, int mode, int xOffset, int yOffset, int width, int height)
  {
      int return_value;
      return_value = pixy_command("cam_getFrame",
      1, mode,
      2, xOffset,
      2, yOffset,
      2, width,
      2, height,
      0,
      &(blk->response), &(blk->fourcc), &(blk->renderflags),
      &(blk->rwidth), &(blk->rheight), &(blk->numPixels), &(blk->pixels),
      0);
    //return return_value;
    return blk->pixels;
  }
  unsigned char getChar(unsigned char * in, int index)
  {
    return*(in+index);
  }
